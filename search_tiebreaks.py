"""One-off: recover how past NBA standings ties were actually broken.
...
Writes nba_tiebreak_orders.json (read by playoff_sim.TIEBREAK_WINNERS).
"""
...
for s in [int(x) for x in sys.argv[2].split(',')]:
    ...
    sim = PS.SeasonSim(s, g[g.season == s], RSG.get(s, 82), conf_of, div_of, ratings, rs_end.get(s))
    ...
    cut = np.sort(pct)[::-1][min(len(pct) - 1, 21)]
...
json.dump({str(k): v for k, v in sorted(out.items())}, open('nba_tiebreak_orders.json', 'w'), indent=1)
